...
function output_page($page_name, $css, $js, $html, $no_index=FALSE) {
	...
	$CI->load->view('page', array("current_page"=>$page_name, "css"=>$css, "js"=>$js, "logged_in"=>$CI->authentication->get_logged_in(), "show_apply"=>$CI->config->item('applications_open'), "show_map_button"=>$CI->config->item('map_enabled'), "recaptcha_lib"=>$CI->recaptcha, "html"=>$html, "login_dialog_html"=>$login_dialog_html, "no_index"=>$no_index), FALSE);
}